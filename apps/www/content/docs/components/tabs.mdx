...
The `prominence` prop controls the visual weight of the tab text. Two values are available: `strong` (default) and `normal`.

- **Strong** – Tabs are displayed in uppercase with semi-bold weight, giving them more visual emphasis.
- **Normal** – Tabs are displayed in regular case with a lighter weight, for a more subtle appearance.

> Note: The `prominence` prop only has a visible effect with the `underlined` variant. When used with the `bordered` variant, the prominence setting will not produce a visual difference.
...
You can customize individual Tabs sub-components by passing Tailwind CSS classes via the `className` prop. The `className` prop also accepts a render function with access to component state (e.g. `isSelected`, `isHovered`, `isDisabled`), allowing you to style tabs based on their current state.
