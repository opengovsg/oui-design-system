...
Selected options always stay in the dropdown list instead of being filtered out, so clicking an already-selected option deselects it again. Set `shouldCloseOnSelect={false}` to also keep the dropdown open after each pick, letting users select several options in one visit without reopening the menu. Pair it with `showCheckbox` to render a checkbox on every option indicating its selected state — the checkbox stays aligned to an option's first line even when its label wraps across multiple lines.

<ComponentPreview name="tag-field-checklist" />
...
- Keyboard interaction: type to filter options; **Up/Down** to navigate the list; **Enter** to select (or deselect, if already selected); **Backspace** on empty input removes the last tag; **Escape** closes the dropdown.
- Each selected tag is a focusable element; **Backspace** or **Delete** on a focused tag removes it.
- With `shouldCloseOnSelect={false}`, the highlighted option stays highlighted after selecting or deselecting it, instead of resetting to the first option in the list.
...
- **`checkboxBox`**: The checkbox square rendered when `showCheckbox` is `true`. Vertically centered against the label's first line so it stays aligned when a label wraps across multiple lines.
- **`checkboxIcon`**: The checkmark icon inside `checkboxBox`, shown when the option is selected.
...
| `itemClassNames`      | `SlotsToClasses<TagFieldItemSlots>`                                                | -                         | Custom Tailwind classes applied to every default `TagFieldItem` row                                                                                     |
...
| `shouldCloseOnSelect` | `boolean`                                                                          | `true`                    | Whether the dropdown closes when an option is selected; set `false` to select multiple options in one visit, with the picked option staying highlighted |
| `showCheckbox`        | `boolean`                                                                          | `false`                   | Whether every option shows a checkbox indicating its selected state; a future major version may default this to `true`                                  |
...
_Inherits `autoFocus` and other focusable props from React Aria's [FocusableProps](https://react-spectrum.adobe.com/react-aria/FocusableProps)._
...
| `classNames`    | `SlotsToClasses<TagFieldItemSlots>` | The resolved slot classes from `itemClassNames`; apply to `container`, `label`, `description`, `checkboxBox`, `checkboxIcon` spans as needed |
...
| `isSelected`    | `boolean`                           | `false` | Whether the item is currently selected; drives the checkbox's checked state when shown                      |
| `classNames`    | `SlotsToClasses<TagFieldItemSlots>` | -       | Custom Tailwind classes for item slots (`container`, `label`, `description`, `checkboxBox`, `checkboxIcon`) |
