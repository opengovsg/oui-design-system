...
Use **Modal** for blocking interactions that require explicit user action or attention. Use [Tooltip](/docs/components/tooltip) for non-blocking hints. Use [Menu](/docs/components/menu) for action lists.
...
_Inherits all remaining props from React Aria's [Dialog](https://react-aria.adobe.com/Dialog). Notable ones: `aria-label`, `aria-labelledby`, `aria-describedby`._
