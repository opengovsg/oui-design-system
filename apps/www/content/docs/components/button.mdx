...
Pass the `preserveWidth` prop to keep the button's width constant while pending,
preventing layout shift. The children are kept in the layout (visually hidden and
hidden from assistive technology) and the spinner is overlaid on top of them.
...
<ComponentPreview name="button-with-preserve-width" />
...
| `preserveWidth`    | `boolean`                                                           | `false`   | Keep the button width constant while pending to prevent layout shift. Only applies when no `loadingText`/`pendingElement` is provided      |
