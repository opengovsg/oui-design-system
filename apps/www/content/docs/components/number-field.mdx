...
Use **NumberField** for numeric input with optional stepper buttons. Use [TextField](/docs/components/text-field) with `type="tel"` or a custom `pattern` if you need digit-only input without steppers.
...
<ShadcnInstall name="number-field" />
